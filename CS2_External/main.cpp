...
void Cheat()
{
  ...
  \_____            ___     ____  |____/
	...
	case 2:
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED);
		cout << "[ERROR] Failed to hook process, please run the cheat as Administrator (Right click SugaR > Run as Adminstrator)." << endl;
		Exit();
	case 3:
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED);
...
int main(void)
{
	...
	if (tempPath != nullptr)
	{
		fileName = std::string(tempPath) + "\\SugaR";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (!otp /*&& !showed*/)
	{
		cout << "Please enter your password! Get the password from: 639798610 or 2279725490, OR you can get free version from:https://aimstar.tkm.icu/ ！" << endl;
		//showed = true;
	}
	...
	switch (message) {
        case WM_CREATE:
	{
		CreateWindowW(L"STATIC", L"Please enter your password:",
	...
	case WM_COMMAND:
	{
		if (LOWORD(wParam) == 1) {
			...
			if (input != code) {
				RetTimes++;
				if (RetTimes < 3) {
					MessageBox(hwnd, L"Password error!!", L"Error", MB_OK | MB_ICONERROR);
				}
				else {
